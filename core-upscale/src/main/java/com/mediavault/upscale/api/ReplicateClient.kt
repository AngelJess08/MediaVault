...
@Singleton
class ReplicateClient @Inject constructor() : UpscaleApiClient {

    override suspend fun submitJob(request: UpscaleRequest): Result<String> {
        return Result.success("replicate_job_${System.currentTimeMillis()}")
    }

    suspend fun submitJob(request: UpscaleRequest, config: UpscaleConfig): String {
        return "replicate_job_${System.currentTimeMillis()}"
    }
