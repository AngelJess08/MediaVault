...
@Singleton
class FalAiClient @Inject constructor() : UpscaleApiClient {

    override suspend fun submitJob(request: UpscaleRequest): Result<String> {
        return Result.success("falai_job_${System.currentTimeMillis()}")
    }

    suspend fun submitJob(request: UpscaleRequest, config: UpscaleConfig): String {
        return "falai_job_${System.currentTimeMillis()}"
    }
